...
- (void)setup
{
    ...
    self.frequency = 1.2f;//1.2
    
    self.amplitude = 1.0f;
    self.idleAmplitude = 0.1f;//0.01
    
    self.numberOfWaves = 5;
    self.phaseShift = -0.25f; //-0.25
    self.density = 0.1f;//1/0.1
...
- (void)setWaverLevelCallback:(void (^)(Waver * waver))waverLevelCallback {
    ...
    ///MARK:    self.displayLink.frameInterval = 30.f; 放在前面 不起作用！！！
    self.displayLink.frameInterval = 5.f;//NSInteger类型的值，用来设置间隔多少帧调用一次selector方法，默认值是1，即每帧都调用一次。

//
//    for(int i=0; i < self.numberOfWaves; i++)
//    {
//        _waveline = [CAShapeLayer layer];
//        _waveline.lineCap       = kCALineCapButt;
//        _waveline.lineJoin      = kCALineJoinRound;
//        _waveline.strokeColor   = [[UIColor clearColor] CGColor];
//        _waveline.fillColor     = [[UIColor clearColor] CGColor];
//        [_waveline setLineWidth:(i == 0 ? self.mainWaveWidth : self.decorativeWavesWidth)];
//
//
//
//        CGFloat progress = 1.0f - (CGFloat)i / self.numberOfWaves;
//        CGFloat multiplier = MIN(1.0, (progress / 3.0f * 2.0f) + (1.0f / 3.0f));
//        UIColor *color = [UICOLOR_RANDOM colorWithAlphaComponent:(i == 0 ? 1.0 : 1.0 * multiplier * 0.4)];
//        _waveline.strokeColor = color.CGColor;
//
//
//
//        [self.layer addSublayer:_waveline];
//        [self.waves addObject:_waveline];
//    }
