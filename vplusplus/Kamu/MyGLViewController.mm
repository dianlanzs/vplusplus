...
- (void)WriteYUVFrame:(Byte *)pYUV Len:(int)length width:(int)w height:(int)h {
    [m_YUVDataLock lock];
//    if(app.m_pYUVData == NULL){
//        app.m_pYUVData = new Byte[length];
//        memset(app.m_pYUVData,0,length);
//    }
    
    memcpy(app.m_pYUVData, pYUV, length);
    W = w;
    H = h;
    m_bHasNewFrame = YES;
